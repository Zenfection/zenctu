...
| Lệnh                      | Chức năng                                                                                                                    |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| ls -l<br>ls -l *dir_name* | Hiển thị tất cả `file/directory` bên trong thư mục hiện tại<br>Hiển thị tất cả `file/directory` bên trong thư mục `dir_name` |
| ls -a                     | Hiển thị tất cả `file/directory` (bao gồm cả `file ẩn`)                                                                      |
| ls -F                     | Thêm vào sau dấu các dấu<br>`*` là file thực thi<br>`@` là *symbolic link*<br>`\|` là *pipe*<br>`>` là *door*                |
| ls -lt                    | Hiển thị sắp xếp theo thời gian chỉnh sửa gần nhất<br>(*tuỳ chọn `-l` cho bạn cái nhìn cụ thể hơn*)                          |
| ls -lh                    | Hiển thị kích thước file đọc được (*như `10M`...*)                                                                           |
| ls -lR                    | Hiển thị tất cả file và thư mục bên trong nó (*đệ quy*)                                                                      |
| tree                      | Ứng dụng cho phép hiển thị các file và thư mục theo dạng cây                                                                 |
